...
サーバーのローカルタイムゾーンは `Asia/Tokyo` に統一する。systemd timer はローカルタイムゾーンの日本時間 AM 8:00 に実行する。
...
1. 画像をクラウドからローカルへコピーする
2. 動画をクラウドからローカルへコピーする
3. ローカルからバックアップ先へ追加コピーする
4. 確認済み動画だけをクラウドから削除する
5. 成功または失敗を Discord へ通知する
...
sequenceDiagram
    ...
    alt ローカルコピーとバックアップが成功
        ...
        Operator->>Discord: 成功通知
    else どちらかが失敗
        Operator->>Log: エラーを記録
        Operator-->>Cloud: 削除しない
        Operator->>Discord: 失敗通知
    end
```
...
- サーバーのローカルタイムゾーンを `Asia/Tokyo` に設定する
- daily timer から単一 service を起動し、画像 copy、動画 copy、バックアップ、確認済み動画削除、Discord 通知を直列実行する
- 削除処理では dry-run 結果をログへ残し、確認済み対象だけを実削除する
- 成功時と失敗時のどちらも Discord へ通知する
