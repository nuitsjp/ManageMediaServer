...
try {
    ...
    if ($ImmichDir.StartsWith("~")) {
        ...
        if ($ImmichDir -eq "~") {
            ...
            # Handles cases like "~anotheruser/path", though less common for $ImmichDir default
            Write-Log "ImmichDir形式 '$ImmichDir' の自動解決は '~/' プレフィックスのみサポートしています。手動で絶対パスを指定してください。" 'WARN'
            $AbsoluteImmichDirWSL = $ImmichDir # Fallback, might not be correct
        }
    } else {
        $AbsoluteImmichDirWSL = $ImmichDir # Assume it's already an absolute WSL path
    }
    # Normalize slashes for WSL (though PowerShell usually handles mixed slashes for wsl commands)
    $AbsoluteImmichDirWSL = $AbsoluteImmichDirWSL.Replace('\', '/')
    Write-Log "Absolute Immich Directory in WSL: $AbsoluteImmichDirWSL"

    # Start-Immich.ps1 スクリプトの内容を定義
    $StartImmichScriptContent = @"
# Start-Immich.ps1 - Dynamically generated by Install-Immich.ps1
Param(
    [string]`$DistroName = '$Distro',
    [string]`$AbsoluteImmichDirWSL = '$AbsoluteImmichDirWSL',
    [string]`$WSLUserName = '$WSLDefaultUser'
)
...
function Write-Log {
    ...
    Write-Host "[``$Timestamp] [``$Level] [Start-Immich] `$Message"
}
...
"@
    ...
    Set-Content -Path $StartImmichScriptPath -Value $StartImmichScriptContent -Encoding UTF8 -Force
    Write-Log "Start-Immich.ps1 スクリプトを '$StartImmichScriptPath' に作成しました。"

    # タスクスケジューラに登録
    $TaskName = "ImmichWSLAutoStart"
    $TaskDescription = "Automatically starts WSL ($Distro) and Immich services at system startup. Managed by Immich installation script."
    # For pwsh.exe, ensure it's in PATH for SYSTEM or use full path: $PSHOME\pwsh.exe
    $PwshPath = (Get-Command pwsh).Source
    if ([string]::IsNullOrEmpty($PwshPath)) {
        Write-Log "pwsh.exe が見つかりませんでした。PowerShell Core がインストールされ、PATHに追加されていることを確認してください。" 'ERROR'
        throw "pwsh.exe not found."
    }

    # Note: Parameters for Start-Immich.ps1 are embedded in its content in this version.
    # If Start-Immich.ps1 were generic, args would be:
    # -Argument "-ExecutionPolicy Bypass -NoProfile -File `"$StartImmichScriptPath`" -DistroName `"$Distro`" -AbsoluteImmichDirWSL `"$AbsoluteImmichDirWSL`" -WSLUserName `"$WSLDefaultUser`""
    # Since they are embedded, we only need to call the script:
    $Action = New-ScheduledTaskAction -Execute $PwshPath -Argument "-ExecutionPolicy Bypass -NoProfile -File `"$StartImmichScriptPath`""
    
    $Trigger = New-ScheduledTaskTrigger -AtStartup
    $Principal = New-ScheduledTaskPrincipal -UserId "NT AUTHORITY\SYSTEM" -LogonType ServiceAccount -RunLevel Highest
    # -ExecutionTimeLimit ([TimeSpan]::Zero) means no time limit.
    $Settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -RunOnlyIfNetworkAvailable:$false -ExecutionTimeLimit ([TimeSpan]::Zero) -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 5) -Compatibility Win8 # Or V1 for broader compatibility if needed.

    # タスクが存在する場合は上書き (-Force)
    ...
    Write-Log "実行ユーザー: NT AUTHORITY\SYSTEM, 最高権限で実行。"

} catch {
    Write-Log "自動起動設定中にエラーが発生しました: $($_.Exception.Message)" 'ERROR'
    Write-Log "タスクスケジューラへの登録は管理者権限が必要です。スクリプトが管理者として実行されているか確認してください。" 'WARN'
    # Continue without failing the whole script, but warn user
}

# --- (既存のスクリプトの最終的な完了メッセージやexitの前にこのセクションを配置) ---
# 例: 元のスクリプトの最後の Write-Log の後
