...
try {
    ...
    $Credential = Get-Credential -UserName $CurrentWindowsUserName -Message "タスク '$TaskName' を '$CurrentWindowsUserName' として登録するためのパスワードを入力してください (テストのため平文変換します):"
    
    # ★★★ SecureStringを平文パスワードに変換 (テスト目的) ★★★
    $PlainTextPassword = ($Credential.GetNetworkCredential()).Password
    # または: $BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($Credential.Password)
    #         $PlainTextPassword = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR)
    #         [System.Runtime.InteropServices.Marshal]::ZeroFreeBSTR($BSTR)


    $TaskDescription = "Automatically starts WSL ($Distro) and Immich services at system startup using $StartImmichScriptName. Runs as $($Credential.UserName)."
    ...
    $Settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -RunOnlyIfNetworkAvailable:$false -ExecutionTimeLimit ([TimeSpan]::Zero) -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 5) -Compatibility Win8

    Write-Log "既存のタスク '$TaskName' があれば削除します..."
    Unregister-ScheduledTask -TaskName $TaskName -Confirm:$false -ErrorAction SilentlyContinue

    Write-Log "タスク '$TaskName' をユーザー '$($Credential.UserName)' で登録試行 (パスワードは平文で渡します)..."
    ...
    Write-Log "スクリプトが管理者権限で実行されているか、指定したユーザー (`$($Credential.UserName)` または `$CurrentWindowsUserName`) のパスワードが正しいか確認してください。" 'WARN'
    # 平文パスワードテストの場合は、その旨をエラーログに残しても良い
    # Write-Log "  (このテストでは平文パスワードを使用しました)" 'DEBUG'
}
