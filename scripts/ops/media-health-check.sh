...
the integrated daily-maintenance timer, and filesystem free space.
...
MAINTENANCE_TIMER="${MAINTENANCE_TIMER:-media-daily-maintenance.timer}"
REQUIRE_MAINTENANCE_TIMER_ENABLED="${REQUIRE_MAINTENANCE_TIMER_ENABLED:-true}"
...
STATE_DIR="${STATE_DIR:-/mnt/data/config/media-health-check/state}"
STATE_FILE="${STATE_FILE:-${STATE_DIR}/last-result}"
...
load_notification_state() {
    ...
    if [[ -f "$STATE_FILE" ]]; then
        PREVIOUS_STATUS=$(awk -F= '$1 == "status" {print $2; exit}' "$STATE_FILE")
        PREVIOUS_SIGNATURE=$(awk -F= '$1 == "signature" {print $2; exit}' "$STATE_FILE")
    fi
}
...
save_notification_state() {
    ...
    [[ "$NO_NOTIFY" == "false" ]] || return 0
    mkdir -p "$STATE_DIR"
    temp_file=$(mktemp "${STATE_DIR}/.last-result.XXXXXX")
    {
        printf 'status=%s\n' "$status"
        printf 'signature=%s\n' "$signature"
    } > "$temp_file"
...
notify_failure_transition() {
    ...
    if [[ "$PREVIOUS_STATUS" != "failed" || "$PREVIOUS_SIGNATURE" != "$signature" ]]; then
        if notify_discord "failed" "one or more health checks failed"; then
            save_notification_state "failed" "$signature"
        fi
    else
        log "Discord failure notification suppressed: failure state is unchanged"
    fi
}

notify_success_transition() {
    if [[ "$PREVIOUS_STATUS" == "failed" ]]; then
        if notify_discord "recovered" "all health checks recovered"; then
            save_notification_state "healthy" ""
        fi
    elif [[ "$NOTIFY_ON_SUCCESS" == "true" ]]; then
        if notify_discord "succeeded" "all health checks passed"; then
            save_notification_state "healthy" ""
        fi
    else
        save_notification_state "healthy" ""
...
check_timer() {
    ...
    if [[ "$REQUIRE_MAINTENANCE_TIMER_ENABLED" == "true" ]]; then
